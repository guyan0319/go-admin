...
import (
	...
	"go-admin/ctrl/admin"
	"go-admin/ctrl/admin/article"
	"go-admin/ctrl/admin/menu"
	"go-admin/ctrl/admin/role"
	"go-admin/ctrl/admin/user"
	"go-admin/ctrl/index"
	"go-admin/lib/cache"
)
...
func AdminStart() {
	...
	r.GET("/getAsyncRoutes", menu.GetAsyncRoutes)
	r.GET("/", admin.Index)
	r.POST("/upload/image", admin.ImgUpload)
	r.GET("/del/image", admin.DelImage)
...
func GetCorsConfig() cors.Config {
	...
	config.AllowOrigins = []string{"http://localhost:8848","http://127.0.0.1:8848"}
...
func Auth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//}
		//uid:=session.Get(v)
		//users := systemdb.SystemUser{Id:uid.(int),Status:1}
		//has:=users.GetRow()
		//if !has {
		//	c.Abort()
